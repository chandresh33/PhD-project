...
def conv_nn(training_x, testing_x, training_y, testing_y, classes):
    ...
    model_cnn.add(Conv1D(32, kernel_size=3, activation='relu', input_shape=(training_x.shape[1], 1)))
    ...
    model_cnn.fit(training_x, train_y, batch_size=32, epochs=15, verbose=1, validation_split=0.2)
...
# df_three = pd.read_csv("Final_dataset\\Filter_specific_data\\cheby_filter_data.csv", header=None, index_col=False)
...
conv_nn(train_, test_, train_labels_, test_labels_, 2)
