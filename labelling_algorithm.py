...
class EllipBesselFilters:
    ...
    def compare_filters(self):
        signal_rep = self.data

        for i in range(len(signal_rep)):
            previous_window = i * self.next_per
            next_window = previous_window + self.next_per
            sig_to_compare = signal_rep.iloc[i, 0:360].values
            ...
            [index_bes, max_pywt, snr_pywt] = self.wavelet_tr(sig_to_compare, clean_sig)
            ...
            if max_pywt >= max_e:
                if max_pywt >= signal_rep.iloc[i, 361]:
                    ...
                    signal_rep.iloc[i, 362] = index_bes
...
def save_file(file_str, full_data):
    os.chdir('C:\\Users\\chand\\PycharmProjects\\Project.py\\Final_dataset')
...
save_file("Completed_data_poiss.csv", data)
print(data.head)
